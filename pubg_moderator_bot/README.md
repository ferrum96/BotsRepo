...
- **Фоновая сверка (каждые 10 минут по умолчанию)** — бот периодически проходит по участникам клана в БД и сверяет их статус в группе (`GROUP_SYNC_INTERVAL_MINUTES`), а также подхватывает в БД завершённые анкеты тех, кто уже состоит в группе.
