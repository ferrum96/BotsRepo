...
EFFECTIVE_PORTFOLIO_PORT="${PORT_PORTFOLIO_PUBLIC:-451}"
...
add_unique_domain "${SERVICE_DOMAIN_ASTROSTONE}"
add_unique_domain "${SERVICE_DOMAIN_PORTFOLIO}"
...
UPSTREAM_PORTFOLIO="127.0.0.1:${EFFECTIVE_PORTFOLIO_PORT}"
...
append_domain_proxy_block "${TMP_CADDYFILE}" "${SERVICE_DOMAIN_ASTROSTONE}" "${UPSTREAM_ASTROSTONE}" "$(cert_fullchain_for "${SERVICE_DOMAIN_ASTROSTONE}")" "$(cert_key_for "${SERVICE_DOMAIN_ASTROSTONE}")"
append_domain_proxy_block "${TMP_CADDYFILE}" "${SERVICE_DOMAIN_PORTFOLIO}" "${UPSTREAM_PORTFOLIO}" "$(cert_fullchain_for "${SERVICE_DOMAIN_PORTFOLIO}")" "$(cert_key_for "${SERVICE_DOMAIN_PORTFOLIO}")"
if [[ "${ENABLE_FKANDU}" = "1" ]]; then
...
if [[ -n "${SERVICE_DOMAIN_PORTFOLIO}" ]]; then
  echo "Portfolio domain: https://${SERVICE_DOMAIN_PORTFOLIO}"
fi
if [[ "${ENABLE_FKANDU}" = "1" ]]; then
