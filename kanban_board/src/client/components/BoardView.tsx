...
export function BoardView({
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      if (idx >= 0 && idx < board.columns.length) {
  ...
  const goToColumn = useCallback((index: number) => {
    if (index < 0 || index >= board.columns.length) return
    setActiveColumnIndex(index)
    programmaticScroll.current = true
    scrollToColumn(index)
    setTimeout(() => { programmaticScroll.current = false }, 400)
  }, [board.columns.length, scrollToColumn])
  ...
  const checkEdgeAndSwitch = useCallback((x: number) => {
    ...
    } else if (x > w - edge && activeColumnIndex < board.columns.length - 1) {
      lastSwitchTime.current = now
      goToColumn(activeColumnIndex + 1)
    }
  }, [isMobile, isDragging, activeColumnIndex, board.columns.length, goToColumn])
  ...
  const handleDragUpdate = (update: DragUpdate) => {
    if (isMobile && update.destination) {
      const destIdx = board.columns.findIndex(c => c.id === update.destination?.droppableId)
  ...
  const handleDragEnd = async (result: DropResult) => {
    ...
    if (isMobile && wasDragging) {
      const targetColumn = board.columns[activeColumnIndex]
  ...
  const handleAddTask = () => {
    setTaskColumnId(board.columns[activeColumnIndex]?.id || board.columns[0]?.id || '')
  ...
  const currentColumn = board.columns[activeColumnIndex]
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden safari-fix-flex">
      <div className="p-4 md:p-6 border-b border-gray-200 bg-white">
        ...
        <div className="mt-4">
          <Filters epics={board.epics} assignees={assignees} filters={filters} onFilterChange={setFilters} />
        </div>
      </div>
      ...
      {isMobile ? (
        <div className="flex-1 flex flex-col min-h-0">
          <div className="px-4 py-2 flex items-center justify-between border-b border-gray-100 bg-white flex-shrink-0">
            ...
            <span className="text-xs text-gray-400">{activeColumnIndex + 1} / {board.columns.length}</span>
          </div>
      ...
      {showTaskModal && (
        <TaskModal task={selectedTask} columnId={taskColumnId} columns={board.columns} epics={board.epics} labels={board.labels}
